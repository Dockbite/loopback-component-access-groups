...
module.exports = class AccessUtils {
  ...
  buildFilter(userId, Model) {
    ...
    return this.getUserGroups(userId)
      .then(userGroups => {
        userGroups = Array.from(userGroups, group => group[this.options.groupKey])
  ...
  getUserGroups(userId, force, cb) {
    ...
    const filter = {where: {role: {inq: this.options.groupRoles.map(gr => this.extractRoleName(gr))}}}
    ...
    Role.registerResolver(accessGroup, (role, context, cb) => {
      ...
      Promise.join(this.getCurrentGroupId(context), this.getTargetGroupId(context),
        (currentGroupId, targetGroupId) => {
          ...
          const actions = [ ]
          const conditions = { role: roleName }
  ...
  getCurrentGroupId(context, cb) {
    ...
    if (context.modelId) {
      ...
      context.model.findById(context.modelId, { }, {
        ...
        .then(item => {
          // TODO: Attempt to follow relationships in addition to the group key.
          if (item) {
            debug(`determined group id ${item[this.options.groupKey]} from existing model ${context.modelId}`)
    ...
    // If we are creating a new model, get the groupKey from the incoming data.
    else if (_get(context, `remotingContext.args.data[${this.options.groupKey}]`)) {
      debug(`determined current group id ${groupId} from incoming data`)
      groupId = context.remotingContext.args.data[this.options.groupKey]
  ...
  getTargetGroupId(context, cb) {
    ...
    if (_get(context, `remotingContext.args.data[${this.options.groupKey}]`)) {
